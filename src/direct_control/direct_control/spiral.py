...
class OffboardControl(Node):
    def __init__(self):
        ...
        # Init Empty List
        self.position_setpoint_list_x = []
        self.position_setpoint_list_y = []
        self.position_setpoint_list_z = []
        self.position_setpoint_list_yaw = []
        self.vehicle_local_position_list_x = []
        self.vehicle_local_position_list_y = []
        self.vehicle_local_position_list_z = []
        self.vehicle_local_position_list_yaw = []
    ...
    def main_loop(self):
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.vehicle_status.arming_state == VehicleStatus.ARMING_STATE_ARMED:
            ...
            while(distance > self.setpoint_max_distance):
                ...
                self.position_setpoint_list_x.append(self.position_setpoint[self.circular_iteration, 0])
                self.vehicle_local_position_list_x.append(self.vehicle_local_position.x)

                self.position_setpoint_list_y.append(self.position_setpoint[self.circular_iteration, 1])
                self.vehicle_local_position_list_y.append(self.vehicle_local_position.y)

                self.position_setpoint_list_z.append(self.position_setpoint[self.circular_iteration, 2])
                self.vehicle_local_position_list_z.append(self.vehicle_local_position.z)

                self.position_setpoint_list_yaw.append(self.position_setpoint[self.circular_iteration, 3])
                # self.vehicle_local_position_list_yaw.append(self.vehicle_local_position.yaw)
                ...
                if self.circular_iteration >= len(self.position_setpoint):
                    self.land()
                    data = {
                        'Setpoint_X': self.position_setpoint_list_x,
                        'Setpoint_Y': self.position_setpoint_list_y,
                        'Setpoint_Z': self.position_setpoint_list_z,
                        'Vehicle_X': self.vehicle_local_position_list_x,
                        'Vehicle_Y': self.vehicle_local_position_list_y,
                        'Vehicle_Z': self.vehicle_local_position_list_z
                    }
                    ...
                    print(len(self.position_setpoint_list_x))
                    print(len(self.position_setpoint_list_y))
                    print(len(self.position_setpoint_list_z))
                    print(len(self.position_setpoint_list_yaw))
                    
                    print(len(self.vehicle_local_position_list_x))
                    print(len(self.vehicle_local_position_list_y))
                    print(len(self.vehicle_local_position_list_z))
                    # print(len(vehicle_local_position_list_yaw))
