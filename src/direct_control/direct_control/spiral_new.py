...
class OffboardControl(Node):
    def __init__(self):
        ...
        self.position_local_list_x = []
        self.position_local_list_y = []
        self.position_local_list_z = []
    ...
    def main_loop(self):
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.vehicle_status.arming_state == VehicleStatus.ARMING_STATE_ARMED:
            ...
            while(distance > self.setpoint_max_distance):
                ...
                self.position_local_list_x.append(self.vehicle_local_position.x)
                self.position_local_list_y.append(self.vehicle_local_position.y)
                self.position_local_list_z.append(self.vehicle_local_position.z)
                ...
                if self.circular_iteration >= len(self.position_setpoint):
                    ...
                    print(len(self.position_local_list_x))
                    print(len(self.position_local_list_y))
                    print(len(self.position_local_list_z))

                    data = {
                        'Setpoint_X': self.position_setpoint_list_x,
                        'Setpoint_Y': self.position_setpoint_list_y,
                        'Setpoint_Z': self.position_setpoint_list_z,
                        'Vehicle_X': self.position_local_list_x,
                        'Vehicle_Y': self.position_local_list_y,
                        'Vehicle_Z': self.position_local_list_z
                    }

                    df = pd.DataFrame(data)

                    df.to_csv('trajectory_new.csv', index=False)
                    ...
                    ax.plot3D(df['Setpoint_X'], df['Setpoint_Y'], df['Setpoint_Z'], 'red')
                    ax.plot3D(df['Vehicle_X'], df['Vehicle_Y'], df['Vehicle_Z'], 'green')
